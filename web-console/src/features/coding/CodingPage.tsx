/**
 * Coding Agent 页面：选择场景模板、启动/继续运行，并显式刷新运行状态与产物。
 *
 * 状态：
 * - templates：scenarioApi.templates 加载 RolePack 列表
 * - scenario / goal：用户选择的模板 id 与 JSON 输入中的 goal 字段
 * - run / artifacts：当前运行快照与产物列表（refresh 时更新）
 * - busy / error：互斥操作锁与错误文案
 *
 * API 流程：
 * - start → POST /scenarios/{id}/start
 * - refresh → GET status + GET artifacts 并行
 * - continueRun → POST .../continue（服务端决定是否允许）
 *
 * 安全：repository 硬编码为 miniclaude-java 仅为 demo；真实部署须由服务端校验工作区白名单。
 * 限制：普通 API 轮询，无 SSE；终端输出为 UI 模拟而非真实 shell 流。
 */
...
export function CodingPage() {
  ...
  /**
   * 启动场景：若未选手动 template 则 fallback 到列表第一项。
   * 成功清空 artifacts；失败保留旧 run 并 setError。
   */
  ...
  /** 并行拉取最新运行状态与产物，更新右侧证据面板（仍为查询时刻快照）。 */
  ...
  /** 请求服务端继续已暂停的场景；前端不预判 status 是否允许 continue。 */
